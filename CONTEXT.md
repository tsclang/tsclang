...
> **Purpose:** Self-contained knowledge dump for AI sessions. Read this FIRST — no need to re-read spec/ unless doing specific work. Last updated: 2026-06-17 (gaps A-E fix from #60, #61-#65).
...
- **safe-math try/catch** — In `safe-math` strict mode, integer arithmetic outside `try { } catch (e: MathError)` or a `throws MathError` function → compile error. Inside try: compiler transforms each integer op to checked version (`__builtin_*_overflow` for +-*-, zero/INT_MIN guard for /%), goto catch on error. `MathError` is a builtin class with `.operation` field. Float arithmetic NOT checked (IEEE 754). `safe-arith`/`safe-div` are deprecated aliases. `Math.checkedAdd/Sub/Mul` removed — use try/catch.
  ...
  - **`!`/`?` in expression context** (`dispatch.js:683-735`): NonNull (`!`) and Propagate (`?`) work inside expressions (`risky()! + 1`, `foo(inner()?)`, `(getData()?).field`). NonNull: Result temp + `tsc_panic`, returns `.value`. Propagate: Result temp + error propagation, returns `.value`. Void returns `((void)0)`. Parser (`parser.js:1508`): `?` disambiguated from ternary via `_isTernaryQuest()` lookahead scanner — scans forward for `:` at same nesting depth (tracking brackets + nested `?`-`:` pairs). Found `:` → ternary. Not found → propagate. `_checkNoBareThrows` (`codegen.js`): recursive check for bare throws calls in binary/array/member/template/argument contexts → compile error. `?`/`!` in async → compile error (use try/catch). Unary `+` supported (identity).
