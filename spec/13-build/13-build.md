...
Актуальная спецификация полей и значений — см. **[13-platform-capabilities.md](13-platform-capabilities.md)**.
...
Профиль загружается из `src/profiles/<name>.json` (встроенные) или из `declare platform` в profile-пакете. См. **[13-platform-capabilities.md](13-platform-capabilities.md)**.
...
**`async_stack`** — максимальный суммарный размер async state machine chain по глубочайшему пути вызовов. Компилятор ошибится если worst-case превышает лимит (см. `--report-stack` в [10-async.md](../10-async/10-async.md)).
...
> **Актуальная спецификация полей `declare platform`** — в [13-platform-capabilities.md](13-platform-capabilities.md). Ниже — исторический контекст и примеры.

Поле `declare platform` описывает возможности целевой платформы. С профилем обязательны поля `allocator` и `async`. Полный список полей, типы, допустимые значения и описания — см. **[13-platform-capabilities.md](13-platform-capabilities.md) → "Референс полей"**.

#### Основные изменения (см. [13-platform-capabilities.md](13-platform-capabilities.md)):
