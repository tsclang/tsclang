...
Тот же механизм `@platform` используется в пользовательском коде (см. [11-concurrency.md](../11-concurrency/11-concurrency.md)).
...
Подробнее о форматах импорта — в [13-build.md](../13-build/13-build.md).
...
Platform profile предоставляет реализацию через `declare module "std/hal" { ... }` — подробнее в [Platform Profile → Структура пакета](../13-build/13-build.md).
