...
| `files` | Файлы для публикации (массив путей) |
...
tsclang install               # установить зависимости из lock-файла (если есть) или создать из tsc.package.json
...
Поле `overrides` — принудительная версия как последний resort при неразрешимом конфликте (уже в `tsc.package.json`).
