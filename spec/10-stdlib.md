...
### Map\<K, V\> на embedded — паттерны замены

`Map<K, V>` требует heap → на `heap: false` платформах (AVR и др.) — ошибка компилятора. Для map-like структур на embedded используются два паттерна:
...
**Когда ни один паттерн не подходит** (большие таблицы, произвольные ключи) — пересмотреть архитектуру: на `heap: false` платформах динамические ассоциативные структуры данных невозможны по определению.
...
const re = new Regex(r"\d{3}-\d{4}")   // compile-time проверка синтаксиса
