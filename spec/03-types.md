...
- **`char`** — внутренний тип компилятора для символьных литералов (`'A'`, `'\n'`) при аннотации `: u8` / `: char`. Эквивалентен `u8`. Без аннотации типа символьные литералы выводятся как `string` (TS compat, см. `spec/02-syntax.md`). Использование `char` в type-аннотациях возможно, но не рекомендуется — используйте `u8`.
...
`'X'` без аннотации = `string` (как в TS). С аннотацией `: u8` / `: char` = char code (uint8_t). Только ASCII и escape-последовательности. Мультибайтовые символы — только в строковых литералах.
