...
### 6.3 ~~`Reader.read(buf: u8[])` — принимает owned буфер~~ ✅ РЕШЕНО

**Решение:** исправлено на `buf: Mut<u8[]>` — буфер передаётся по мутабельной ссылке, не перемещается. Один буфер можно переиспользовать в цикле.
...
### 7.1 ~~Mandatory formatting — ломает incremental compilation в IDE~~ ✅ РЕШЕНО

**Решение:** форматирование — никогда не ошибка компилятора. Отдельная команда `tsclang format` / `tsclang format --check` (для CI). Language server работает независимо от форматирования. Подробно — CONCEPT.md §"Форматирование".
