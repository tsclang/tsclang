...
static void Dog_speak(Dog *self) { printf("woof\n"); }
static void Cat_speak(Cat *self) { printf("meow\n"); }
...
static const Animal_vtable *_Cat_vtable_ptr = &_Cat_Animal_vtable;

int main(void) {
    TSC_INIT();
    Dog _dog_0 = {0};
    Animal a = { .self = &_dog_0, .vtable = &_Dog_Animal_vtable };
    if (a.vtable == _Cat_vtable_ptr) {
        printf("cat\n");
    } else {
        printf("not cat\n");
    }
