...
static void a_poll(a_state *self) {
    switch (self->_state) {
        case 0: self->_result = (Result_i32_Err){.ok = true, .value = 1}; self->_done = true; return;
    }
}
...
static void b_poll(b_state *self) {
    switch (self->_state) {
        case 0: self->_result = (Result_i32_Err){.ok = false, .error = Err_new(STR_LIT("fail"))}; self->_done = true; return;
    }
}

typedef struct {
    int32_t _state; int _result; bool _done;
    a_state _await_0; b_state _await_1;
} run_state;
...
static void run_poll(run_state *self) {
    switch (self->_state) {
        ...
        case 1:
            ...
            if (!self->_await_0._result.ok) { printf("error\n"); self->_done = true; return; }
            if (!self->_await_1._result.ok) { printf("error\n"); self->_done = true; return; }
            self->_done = true;
            return;
    }
}
