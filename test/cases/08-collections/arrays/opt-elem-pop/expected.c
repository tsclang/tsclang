...
int main(void) {
    TSC_INIT();
    opt_i32 _lit_0[] = {((opt_i32){true, 42}), ((opt_i32){false, 0})};
    Array_opt_i32 arr = {.data = _lit_0, .length = 2, .capacity = 2};
    opt_i32 a = tsc_array_pop_opt_i32(&arr);
    opt_i32 b = tsc_array_pop_opt_i32(&arr);
    printf("%d\n", a.value);
    printf("%d\n", b.value);
    return 0;
}
