...
typedef struct { int32_t _state; int32_t _result; bool _done; int32_t x; } checkItem_state;

static void checkItem_poll(checkItem_state *self) {
    switch (self->_state) {
        case 0:
            self->_result = self->x;
            self->_done = true;
...
case_1:
        case 1:
            ...
            self->_await_0 = (checkItem_state){0};
            self->_state = 2;
            /* fall through */
        case 2:
            checkItem_poll(&self->_await_0);
            if (!self->_await_0._done) return;
            int32_t v = self->_await_0._result;
            if (v == self->target) goto forof_1_end;
forof_1_cont:
            self->_forof_idx_0++;
            if (!(self->_forof_idx_0 < self->arr.length)) { goto forof_1_end; }
            self->_state = 1;
