...
typedef struct { bool ok; union { opt_ref_Particle value; TscError error; }; } Result_opt_ref_Particle_TscError;
...
Result_opt_ref_Particle_TscError make(void) {
    ...
    if (!_pool_0.has_value) {
        return (Result_opt_ref_Particle_TscError){.ok = false, .error = (TscError){ .message = STR_LIT("pool exhausted: Particle") }};
    }
