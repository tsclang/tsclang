...
static void _tsc_unknown_drop_Point(void *buf) { Point *ptr; memcpy(&ptr, buf, sizeof(ptr)); free(ptr); }
static void _tsc_unknown_clone_Point(const void *src, void *dst) { Point *sp; memcpy(&sp, src, sizeof(Point*)); Point *dp = (Point*)malloc(sizeof(Point)); *dp = *sp; memcpy(dst, &dp, sizeof(dp)); }
static const tsc_unknown_vtable _tsc_vt_Point = {_tsc_unknown_drop_Point, _tsc_unknown_clone_Point};
static inline tsc_unknown tsc_unknown_from_Point(Point obj) { Point *heap = (Point*)malloc(sizeof(Point)); *heap = obj; tsc_unknown u = {.type_id = 8, .vtable = &_tsc_vt_Point}; memcpy(u.buffer, &heap, sizeof(heap)); return u; }
static inline Point* tsc_unknown_get_Point(const tsc_unknown *self) { Point *ptr; memcpy(&ptr, self->buffer, sizeof(ptr)); return ptr; }
