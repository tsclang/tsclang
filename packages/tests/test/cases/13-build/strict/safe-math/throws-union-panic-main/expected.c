...
Result_i32_MathError_IOError _tsc_main(void) {
    MathError _func_math_err = {0};
    Result_i32_MathError_IOError _res_1 = risky_i32_i32(1, 2);
    if (!_res_1.ok) { return (Result_i32_MathError_IOError){.ok = false, .error = _res_1.error}; }
    int32_t z = _res_1.value;
    return (Result_i32_MathError_IOError){.ok = true, .value = z};
    _func_math_throw:
        _ErrUnion_MathError_IOError _math_union = {.tag = _Err_MathError, ._0 = _func_math_err};
        return (Result_i32_MathError_IOError){.ok = false, .error = _math_union};
}
