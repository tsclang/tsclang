...
typedef struct { int32_t _state; int _result; bool _done; a_state _await_0; b_state _await_1; } run_state;
...
static void run_poll(run_state *self) {
    switch (self->_state) {
        ...
        case 1:
            ...
            int32_t x = self->_await_0._done ? self->_await_0._result : self->_await_1._result;
            printf("%d\n", x);
            self->_done = true;
            return;
    }
