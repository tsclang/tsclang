...
Выполняется после парсинга, до проверки типов. Подробно описан в [15-decorators.md](../15-decorators/15-decorators.md).
...
Формальная схема описана в [06-name-mangling.md](../06-functions/06-name-mangling.md) (раздел «Name mangling — формальная схема»). Здесь — замечания для реализации компилятора.
...
### Каталог warnings (W001-W011)

| Код | Триггер | Файл |
|-----|---------|------|
| W001 | class/Array/Map/Set в boolean context — всегда truthy | `dispatch.ts` |
| W002 | `!expr` где expr всегда truthy — dead code | `dispatch.ts` |
| W003 | switch по enum без покрытия всех вариантов (без default) | `control-flow.ts` |
| W004 | `native` блок — C код вставлен как есть, без проверок | `control-flow.ts` |
| W005 | `unsafe` блок — ownership и type checks отключены | `control-flow.ts` |
| W006 | Прямая рекурсия в async функции — heap allocation required | `async-emit.ts` |
| W007 | `@struct` с неэффективным порядком полей (padding ≥4 bytes, >20%) | `class.ts` |
| W008 | Move из `const` binding в strict mode (без strict → E003) | `assign.ts`, `vardecl.ts`, `call-dispatch.ts` |
| W009 | `mut` метод на классе где ВСЕ поля readonly | `decorators.ts` |
| W010 | `AsyncMutex.lock()` в async функции (блокирующий вызов) | `concurrency.ts` |
| W011 | `i64`/`u64`/`f64`/`f32` на 8-bit платформе (дорого) | `vardecl.ts` |
## Test Engine API (��� ���������� ����������� � ����������������� ������������)

Jest-like API ��� ��������� ������ � TypeScript. ����� ����������� TSClang > C > binary � ��������� ���������.
...
| `describe(name, fn)` | ����������� ������ |
| `test(name, callback)` | ������ ����� |
| `run(code, opts?)` | ����������� TSC > C > binary, ���������� stdout |
| `compile(codeOrPath)` | ����������� TSC > C, ���������� C ������ |
| `file(path)` | ������ .tsc ���� |
| `expect(value)` | ������ chain � ��������� |
| `expect(() => ...)` | ������ chain ��� �������� throw |
| `TscError / CompileError / RuntimeError` | ������ ������ �� ����� |
