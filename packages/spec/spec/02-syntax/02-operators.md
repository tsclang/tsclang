...
| `/` | деление |
| `%` | остаток от деления |
| `**` | возведение в степень — всегда через `pow()` из `math.h`, результат `f64`. Для integer-операндов требуется явное приведение: `a ** b as i32` |
...
| Оператор | Описание |
|----------|----------|
| `==` / `===` | равенство (нет type coercion; `===` — алиас для `==`, рекомендуется для ясности) |
| `!=` / `!==` | неравенство (нет type coercion; `!==` — алиас для `!=`, рекомендуется для ясности) |
| `<` | меньше |
| `>` | больше |
| `<=` | меньше или равно |
| `>=` | больше или равно |

> В TSC нет неявного приведения типов, поэтому `==` и `===` (также `!=` и `!==`) ведут себя одинаково. Рекомендуется `===` / `!==` для ясности намерений.
