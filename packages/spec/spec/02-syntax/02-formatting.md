...
**Автоматическая вставка точек с запятой (ASI)** — компилятор автоматически вставляет `;` когда следующая строка **не может** быть продолжением выражения.

**Символы, которые НЕ вызывают ASI** (следующая строка интерпретируется как продолжение выражения):
...
**Символы, которые ВЫЗЫВАЮТ ASI** (начинают новое выражение):

| Символ | Поведение |
|--------|----------|
| `+` / `-` | Бинарный оператор: `a + b` (не унарный `+b`) |
| `.` | Chain access: `a.b` |
| `++` / `--` | Инкремент/декремент |

> **Важно:** `+` и `-` на новой строке — это **бинарные** операторы, не унарные. Если нужно унарное действие, используйте `;`:
> ```typescript
> const a = 2
> + 3  // ✅ b = 5 (бинарный)
> 
> const c = 2;
> +3  // ⚠️ унарный плюс, dead code
> ```
