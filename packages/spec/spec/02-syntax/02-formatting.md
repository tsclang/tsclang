...
| `+` / `-` / `*` / `/` | Бинарные операторы |
| `%` | Остаток от деления |
| `.` | Chain access: `a.toString()` |
| `++` / `--` | Инкремент/декремент |

> **Примечание:** `.` на новой строке — частный случай. Компилятор интерпретирует его как продолжение выражения (`a.toString()`), но если результат не используется, генерируется dead code. Для корректного поведения — используйте `;`.
