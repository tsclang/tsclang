...
**Автоматическая вставка точек с запятой (ASI)** — компилятор автоматически вставляет `;` когда следующая строка **не может** быть продолжением выражения. Но есть важное исключение:

**Унарные операторы `+`, `-` на новой строке** — интерпретируются как **продолжение** предыдущего выражения (бинарный оператор), а не как унарный:

```typescript
const a = 1
[1, 2].forEach(x => console.log(x))  // ❌ error: [ интерпретируется как индекс a[1]

const a = 1;                           // ✅ ok — ; разделяет
[1, 2].forEach(x => console.log(x))

const b = 2
+ 3  // ✅ ok: + интерпретируется как бинарный оператор → b = 5

const c = 2 + 3  // ✅ ok — оператор на той же строке
```

> **Важно:** Если поставить `;` после `2`, то `+3` станет отдельным statement и будет интерпретирован как **унарный плюс**:
> ```typescript
> const c = 2;
> +3  // ⚠️ +3 = унарный оператор, dead code (результат не используется)
> ```
> Это может привести к скрытым багам.

> **Примечание:** `.` на новой строке — интерпретируется как continuation выражения (`a.b`), а не как отдельный statement. Это может привести к скрытым багам: developer хотел `.c` на отдельной строке, а получил доступ к полю объекта. Для явного разделения — используйте `;`.
