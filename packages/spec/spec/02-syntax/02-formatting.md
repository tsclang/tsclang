...
**Автоматическая вставка точек с запятой (ASI)** — работает только когда следующая строка **не может** быть продолжением выражения. Не работает если следующая строка начинается с символов, которые могут продолжить предыдущее выражение:

| Символ | Почему не работает |
|--------|-------------------|
| `[` | Может быть индексом: `a[0]` |
| `(` | Может быть вызовом: `a(b)` |
| `` ` `` | Template literal: `a\`b\`` |
| `+` / `-` / `*` / `/` | Бинарные операторы |
| `%` | Остаток от деления |
| `.` | Chain access: `a.toString()` — но на новой строке интерпретируется как continuation выражения (`a.b`), а не как отдельный statement |
| `++` / `--` | Инкремент/декремент |

> **Примечание:** `.` на новой строке — частный случай. Компилятор интерпретирует его как продолжение выражения (`a.b`), а не как отдельный statement. Это может привести к скрытым багам: developer хотел `.c` на отдельной строке, а получил доступ к полю объекта. Для явного разделения — используйте `;`.
...
const b = 2
+ 3  // ❌ error: + интерпретируется как унарный плюс к 3, бинарное выражение
