...
### Match в expression context

`match` может использоваться не только как RHS переменной, но и внутри более крупных выражений — аргументов функций, бинарных операций, return:

```typescript
// как аргумент функции
console.log(match (x) { 1 => "one", _ => "many" });

// в бинарном выражении
const total = 10 + match (x) { 1 => 100, _ => 0 };
...
Под капотом компилятор создаёт временную переменную, генерирует if/else цепочку (через общий код с match-statement), и использует значение переменной в окружающем выражении.
