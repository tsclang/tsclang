...
**Именованные функции как колбэки:** кроме inline-стрелок, можно передавать именованные функции. Компилятор выводит тип результата из сигнатуры функции:

```typescript
function scale(x: i32): f64 {
    return x * 2.0
}
const nums: i32[] = [1, 2, 3]
const scaled = nums.map(scale)          // f64[] — [2.0, 4.0, 6.0]
const chained = nums.map(scale).filter(x => x > 3.0)  // f64[] — [4.0, 6.0]
```

> **Ограничение:** именованные функции-колбэки для массивов `string[]` пока не поддерживаются — runtime-макросы передают строковые элементы по указателю (`String *`), а именованные функции принимают `String` по значению. Используйте стрелочные функции для строковых массивов.
