...
- Полный набор: `i8`, `i16`, `i32`, `i64`, `u8`, `u16`, `u32`, `u64`, `isize`, `usize`, `f32`, `f64`, `d8`, `d16`, `d32`, `d64`
- Decimal fixed-point типы (`d8`/`d16`/`d32`/`d64`) — integer storage с scale factor, exact decimal arithmetic без FPU. Подробно: [03-decimal-types.md](./03-decimal-types.md).
...
**Для литералов:** правило то же самое. Литерал без аннотации получает тип number = defaultNumber. При бинарной операции два литерала приводятся к наибольшему типу:

```typescript
let a = 1 + 2;         // number (defaultNumber, f64 на desktop)
let b = 1 + 2.5;       // f64 (2.5 - float, выигрывает)
let c: i32 = 1;        // i32 (явная аннотация)
let d = c + 2;         // i32 (c: i32, литерал 2 приводится к i32)
let e = c + 2.5;       // f64 (2.5 - float, выигрывает)
```
