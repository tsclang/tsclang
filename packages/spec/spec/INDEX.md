...
| **Strict Mode** | Granular compile-time правила: `no-any`, `no-unsafe`, `no-native`, `safe-math`, `no-lossy-cast`, `no-dynamic-alloc`. Для safety-critical кода (IEC 61508). |
...
Запуск: `pnpm tsx packages/tests/test/runner.ts 03-types` (фильтр по разделу или фиче).
...
| `03-types` | 464 | Числа, decimal fixed-point, enum, type aliases, tuples, utility types, null/optional, unknown, as-operator, widening, char, usize, Date |
