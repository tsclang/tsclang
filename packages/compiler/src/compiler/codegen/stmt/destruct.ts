import type { Expression, TypeAnn, ObjLitProp, VarDestructObj, VarDestructArr } from '@tsclang/ast';
...
export function _visitVarDestruct(ctx: CodeGenContext, node: VarDestructObj | VarDestructArr, lines: string[], depth: number) {
    const I = ' '.repeat(ctx.indent * depth);
    const p = (s: string) => lines.push(I + s);
    if (node.kind === 'VarDestructObj') {
        const { varKind, typeAnn, init } = node;
        const pattern = node.pattern as unknown as ObjDestructField[];
        const qual = varKind === 'const' ? 'const ' : '';
        const objType = ctx.inferType(init);
        const structDef = ctx.classes.get(objType);

        // Deferred anon struct (from ObjLit Ident): expand props directly
        if (init.kind === 'Ident') {
          const _dSym = ctx.lookup(init.name);
          if (_dSym?.deferredAnon && ctx._deferredAnons?.has(init.name)) {
            const _dAnon = ctx._deferredAnons.get(init.name)!;
            ...
            for (const { name: fname } of _dAnon.fields) {
              ...
              const propC2 = propVal2 ? ctx.exprToC(propVal2, lines, depth) : '0';
              const propType2 = propVal2 ? ctx.inferType(propVal2) : 'int32_t';
              p(`${qual}${propType2} _obj_${fname} = ${propC2};`);
              ctx.define(`_obj_${fname}`, { ctype: propType2, varKind });
            }
            for (const { name: fname, alias, defaultVal } of pattern) {
              const propType2 = ctx.lookup(`_obj_${fname}`)?.ctype ?? 'int32_t';
              if (defaultVal) {
                const dC2 = ctx.exprToC(defaultVal, lines, depth);
                p(`${qual}${propType2} ${alias} = (_obj_${fname} != 0) ? _obj_${fname} : ${dC2};`);
              } else {
                p(`${qual}${propType2} ${alias} = _obj_${fname};`);
              }
            ctx.define(alias, { ctype: propType2, varKind });
            }
            return;
          }
        }
        ...
        if (init.kind === 'ObjLit') {
          ...
          for (const { name } of pattern) {
            ...
            const propC = propVal ? ctx.exprToC(propVal, lines, depth) : '0';
            const propType = propVal ? ctx.inferType(propVal) : 'int32_t';
            p(`${qual}${propType} _obj_${name} = ${propC};`);
            ctx.define(`_obj_${name}`, { ctype: propType, varKind });
          }
          // Second pass: bind destructured names
          for (const { name, alias, defaultVal } of pattern) {
            const propType = ctx.lookup(`_obj_${name}`)?.ctype ?? 'int32_t';
            if (defaultVal) {
              const dC = ctx.exprToC(defaultVal, lines, depth);
              p(`${qual}${propType} ${alias} = (_obj_${name} != 0) ? _obj_${name} : ${dC};`);
            } else {
              p(`${qual}${propType} ${alias} = _obj_${name};`);
            }
            ctx.define(alias, { ctype: propType, varKind });
          }
          return;
        }
        ...
        if (typeAnn && init.kind === 'Ident' && structDef?.fields) {
          ...
          for (const { name, alias } of pattern) {
            ...
            const fieldCType = field?.typeAnn ? ctx.resolveType(field.typeAnn) : 'int32_t';
            ...
            ctx.define(alias, { ctype: fieldCType, varKind });
            if (fieldCType === 'String') {
              ctx._registerCleanup(`tsc_string_release(${alias})`);
            }
          }
        ...
        if (init.kind === 'Ident' && structDef?.fields) {
          ...
          for (const { name, alias } of pattern) {
            ...
            ctx.define(alias, { ctype: `${fieldCType} *`, varKind, isPointer: true, derefType: fieldCType });
          }
          return;
        }
        ...
        const tmpName = `_obj_${ctx.tempCount++}`;
        ...
        for (const { name, alias, defaultVal } of pattern) {
          ...
          ctx.define(alias, { ctype: 'int32_t', varKind });
        }

    } else if (node.kind === 'VarDestructArr') {
        const { varKind, typeAnn, init } = node;
        const pattern = node.pattern as unknown as (ArrDestructElem | null)[];
        let initType = ctx.inferType(init);
        ...
        let tupleDef0 = ctx.classes.get(initType);
        ...
        if (!tupleDef0?.isTuple && !initType?.startsWith('Array_') && typeAnn) {
          const resolved = ctx.resolveType(typeAnn);
          const resolvedDef = ctx.classes.get(resolved);
        ...
        if (tupleDef0?.isTuple) {
          const initC = ctx.exprToC(init, lines, depth);
          const srcIsLet = init?.kind === 'Ident' && ctx.lookup(init.name)?.varKind === 'let';
          for (let i = 0; i < pattern.length; i++) {
            ...
            ctx.define(elem.name, { ctype, varKind });
            if (ctype === 'String') {
          ...
          const elemCType = ctx._arrIdentToCType(elemIdent);
          const srcIsLet = init?.kind === 'Ident' && ctx.lookup(init.name)?.varKind === 'let';
          ctx._ensureArrayStruct(initType, elemCType);
          const srcC = ctx.exprToC(init, lines, depth);
          ...
          for (const elem of pattern) {
            ...
            if (elem.rest) {
              ...
              ctx.define(elem.name, { ctype: initType, elemType: elemIdent, arrElemCType: elemCType, isArray: true, varKind });
              ctx._registerCleanup(`tsc_array_free_${elemIdent}(&${elem.name})`);
            } else {
              // Regular element: direct index
              p(`${qual}${elemCType} ${elem.name} = ${srcUse}.data[${idx}];`);
              ctx.define(elem.name, { ctype: elemCType, varKind });
              if (elemCType === 'String') {
                p(`tsc_string_retain(${elem.name});`);
                ctx._registerCleanup(`tsc_string_release(${elem.name})`);
              }
              idx++;
            }
          }
          ...
          const initC = ctx.exprToC(init, lines, depth);
          const tmpName = `_arr_${ctx.tempCount++}`;
          ...
          for (let i = 0; i < pattern.length; i++) {
            ...
            if (elem.rest) {
              ...
              ctx.define(elem.name, { ctype: 'int32_t', varKind });
            }
          }
        }
