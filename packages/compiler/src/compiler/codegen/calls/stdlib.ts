import type { CodeGenContext } from '../../codegen.js';
import type { Call, Argument, Param, SymbolInfo } from '@tsclang/ast';
import { DEFAULT_CONSOLE_BAUD } from '@tsclang/shared';

export function _dispatchStdLib(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    let _r;
    _r = ctx._dispatchStdIo(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdHal(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdBlob(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdUrl(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdSignal(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdWs(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdNet(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdFs(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdTemporal(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdBuffer(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdDataView(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdHashMap(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdSet(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdTasks(node, lines, depth);
    if (_r !== null) return _r;
    _r = ctx._dispatchStdRegex(node, lines, depth);
    if (_r !== null) return _r;
    return null;
}

export function _dispatchStdIo(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    if (ctx._stdIoImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _ioSym = ctx.lookup(callee.object.name);
      const _ioName = callee.object.name;
      const _ioProp = callee.prop;
      if (_ioSym?.ctype === 'Reader' && _ioProp === 'read') {
        const bufArg = args[0]?.expr;
        const bufName = bufArg?.kind === 'Ident' ? bufArg.name : ctx.exprToC(bufArg, lines, depth);
        ctx._lastHalRead = 'size_t';
        return `${_ioName}.vtable->read(${_ioName}.self, ${bufName}.data, ${bufName}.length)`;
      }
      if (_ioSym?.ctype === 'Writer' && _ioProp === 'write') {
        const bufArg = args[0]?.expr;
        const bufName = bufArg?.kind === 'Ident' ? bufArg.name : ctx.exprToC(bufArg, lines, depth);
        return `${_ioName}.vtable->write(${_ioName}.self, ${bufName}.data, ${bufName}.length)`;
      }
    }
    return null;
}

export function _dispatchStdHal(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    if (ctx._stdHalImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _halClass = callee.object.name;
      const _halProp  = callee.prop;
      if (_halClass === 'GPIO') {
        if (_halProp === 'mode') {
          const pin  = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          const mode = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : 'TSC_PINMODE_INPUT';
          return `tsc_gpio_mode(${pin}, ${mode})`;
        }
        if (_halProp === 'output') return `tsc_gpio_output(${args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0'})`;
        if (_halProp === 'input')  return `tsc_gpio_input(${args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0'})`;
        if (_halProp === 'write') {
          const pin = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          const val = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : 'false';
          return `tsc_gpio_write(${pin}, ${val})`;
        }
        if (_halProp === 'read') {
          ctx._lastHalRead = 'bool';
          return `tsc_gpio_read(${args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0'})`;
        }
      }
      if (_halClass === 'I2C') {
        if (_halProp === 'begin') return 'tsc_i2c_begin()';
        if (_halProp === 'write') {
          const addr = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          const dataN = args[1]?.expr?.kind === 'Ident' ? args[1].expr.name : '_data';
          return `tsc_i2c_write(${addr}, ${dataN}.data, ${dataN}.length)`;
        }
        if (_halProp === 'read') {
          const addr = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          const len  = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : '0';
          ctx._lastHalRead = 'Array_u8';
          return `tsc_i2c_read(${addr}, ${len})`;
        }
      }
      if (_halClass === 'SPI') {
        if (_halProp === 'begin') return 'tsc_spi_begin()';
        if (_halProp === 'transfer') {
          ctx._lastHalRead = 'uint8_t';
          return `tsc_spi_transfer(${args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0'})`;
        }
      }
      if (_halClass === 'UART') {
        if (_halProp === 'init') {
          const cfgArg = args[0]?.expr;
          let baud = String(DEFAULT_CONSOLE_BAUD);
          if (cfgArg?.kind === 'ObjLit') {
            const bp = cfgArg.props?.find((p) => p.key === 'baud');
            if (bp?.value) baud = ctx.exprToC(bp.value, lines, depth);
          }
          return `tsc_uart_init(${baud})`;
        }
        if (_halProp === 'available') {
          ctx._lastHalRead = 'bool';
          return 'tsc_uart_available()';
        }
        if (_halProp === 'write') return `tsc_uart_write(${args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0'})`;
        if (_halProp === 'read') {
          ctx._lastHalRead = 'opt_u8';
          ctx._ensureOptStruct('opt_u8', 'uint8_t');
          return `tsc_uart_read()`;
        }
      }
    }

    return null;
}

export function _dispatchStdBlob(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    if (callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _blobSym = ctx.lookup(callee.object.name);
      const _blobName = callee.object.name;
      const _blobProp = callee.prop;
      if (_blobSym?._isBlob) {
        if (_blobProp === 'slice') {
          const start = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          const end   = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : `${_blobName}.size`;
          const sizeExpr = (args[0] && args[1] &&
            args[0].expr.kind === 'Literal' && args[1].expr.kind === 'Literal')
            ? String(parseInt(args[1].expr.value) - parseInt(args[0].expr.value))
            : `(${end} - ${start})`;
          return `{.data = ${_blobName}.data + ${start}, .size = ${sizeExpr}}`;
        }
        if (_blobProp === 'arrayBuffer') {
          // Ensure Buffer typedef
          if (!ctx._emittedBufferTypeDef) {
            ctx._emittedBufferTypeDef = true;
            ctx.addTop('typedef struct { uint8_t *data; size_t length; } Buffer;');
            ctx.addTop('');
          }
          return `{.data = ${_blobName}.data, .length = ${_blobName}.size}`;
        }
      }
      if (_blobSym?._isTscBlob) {
        if (_blobProp === 'text') {
          const n = ctx._blobTextN = (ctx._blobTextN ?? 0); ctx._blobTextN++;
          const tmp = `_text_${n}`;
          const I = ' '.repeat(ctx.indent * depth);
          lines.push(`${I}String ${tmp} = tsc_blob_text(&${_blobName});`);
          ctx._pushPostStmtCleanup(`${I}tsc_string_release(${tmp});`);
          return tmp;
        }
      }
    }
    return null;
}

export function _dispatchStdUrl(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // URL / URLSearchParams method calls
    if (ctx._stdUrlImported && callee.kind === 'Member') {
      // u.searchParams.get/set/delete вЂ” callee.object is Member (u.searchParams)
      if (callee.object.kind === 'Member' && callee.object.prop === 'searchParams') {
        const urlObj = callee.object.object;
        const urlSym = urlObj.kind === 'Ident' ? ctx.lookup(urlObj.name) : null;
        if (urlObj.kind === 'Ident' && urlSym?._isURL) {
          const urlName = urlObj.name;
          const _spProp = callee.prop;
          if (_spProp === 'get') {
            const keyC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
            const n = ctx._urlOptN = (ctx._urlOptN ?? 0); ctx._urlOptN++;
            const tmp = `_v_${n}`;
            lines.push(`${' '.repeat(depth * 4)}TscOptString ${tmp} = tsc_search_params_get(&${urlName}.searchParams, ${keyC});`);
            return `${tmp}.has_value ? ${tmp}.value.data : "null"`;
          }
          if (_spProp === 'set') {
            const k = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
            const v = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : 'STR_LIT("")';
            return `tsc_url_params_set(&${urlName}, ${k}, ${v})`;
          }
          if (_spProp === 'delete') {
            const k = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
            return `tsc_url_params_delete(&${urlName}, ${k})`;
          }
        }
      }
      // p.get(key) вЂ” URLSearchParams standalone
      if (callee.object.kind === 'Ident') {
        const _spSym = ctx.lookup(callee.object.name);
        if (_spSym?._isURLSearchParams) {
          const _spName = callee.object.name;
          const _spProp = callee.prop;
          if (_spProp === 'get') {
            const keyC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
            const n = ctx._urlOptN = (ctx._urlOptN ?? 0); ctx._urlOptN++;
            const tmp = `_v_${n}`;
            lines.push(`${' '.repeat(depth * 4)}TscOptString ${tmp} = tsc_search_params_get(&${_spName}, ${keyC});`);
            return `${tmp}.has_value ? ${tmp}.value.data : "null"`;
          }
        }
      }
    }
    return null;
}

export function _dispatchStdSignal(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // Signal methods: signal.get(), signal.set(val)
    if (ctx._stdReactiveImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _sigSym = ctx.lookup(callee.object.name);
      if (_sigSym?._isSignal) {
        const etIdent = _sigSym._signalElemType;
        const sigRef = ctx._capturedSignalMap?.get(callee.object.name) ?? `&${callee.object.name}`;
        if (callee.prop === 'get') return `tsc_signal_get_${etIdent}(${sigRef})`;
        if (callee.prop === 'set') {
          if (ctx._inComputedFn) throw ctx.errorCode('E418', null, { detail: 'TypeError: Side effect (signal.set) inside computed() is not allowed' });
          const val = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_signal_set_${etIdent}(${sigRef}, ${val})`;
        }
      }
    }

    // computed(arrow), effect(arrow), batch(arrow)
    if (ctx._stdReactiveImported && callee.kind === 'Ident') {
      if (callee.name === 'computed' || callee.name === 'effect' || callee.name === 'batch') {
        const arrow = args[0]?.expr;
        const n = callee.name === 'batch' ? (ctx._batchCount ?? 0) : (ctx._reactiveClosureCount ?? 0);
        const prefix = callee.name === 'batch' ? `_batch_${n}` : `_closure_${n}`;
        const fnName = `${prefix}_fn`;

        if (callee.name === 'batch') {
          // batch: emit static fn that accesses already-captured signal refs
          ctx._batchCount = (ctx._batchCount ?? 0) + 1;
          const batchLines: string[] = [];
          if (arrow?.kind === 'Arrow') {
            // Use persistent capture refs inside batch body
            const _savedMap = ctx._capturedSignalMap;
            ctx._capturedSignalMap = new Map(ctx._persistentCaptureRefs ?? []);
            ctx.pushScope();
            if (arrow.body.kind === 'Block') ctx.visitBlock(arrow.body, batchLines, 0);
            else { const c = ctx.exprToC(arrow.body, batchLines, 0); batchLines.push(`return ${c};`); }
            ctx.popScope();
            ctx._capturedSignalMap = _savedMap;
          }
          ctx.topLevel.push(`static void ${fnName}(void) {`);
          for (const l of batchLines) ctx.topLevel.push('    ' + l);
          ctx.topLevel.push('}');
          ctx.topLevel.push('');
          return `tsc_batch(${fnName})`;
        }

        // computed/effect: collect Signal free vars, emit env struct + static captured + fn
        if (!arrow || arrow.kind !== 'Arrow') return `tsc_effect(NULL)`;
        ctx._reactiveClosureCount = (ctx._reactiveClosureCount ?? 0) + 1;

        // Find free Signal vars
        const paramNames = new Set((arrow.params ?? []).map((p: { name?: string }) => p.name));
        const capturedSignals = new Map();
        const walkFreeVars = (node: unknown): void => {
          if (!node || typeof node !== 'object') return;
          if (Array.isArray(node)) { (node as unknown[]).forEach(walkFreeVars); return; }
          const n = node as Record<string, unknown>;
          if (n.kind === 'Call') {
            const callee = n.callee as Record<string, unknown> | undefined;
            const obj = callee?.object as Record<string, unknown> | undefined;
            if (callee?.kind === 'Member' && obj?.kind === 'Ident') {
              const varName = obj.name as string;
              if (!paramNames.has(varName)) {
                const sym = ctx.lookup(varName);
                if (sym?._isSignal) capturedSignals.set(varName, sym);
              }
            }
          }
          for (const key of Object.keys(n)) {
            if (key !== 'kind') walkFreeVars(n[key]);
          }
        };
        walkFreeVars(arrow.body);

        // Emit env struct and static captured:
        // if topLevel has global statics already в†’ append there; else use typedefs (stays adjacent to Signal typedef)
        let _envInTopLevel = false;
        if (capturedSignals.size > 0) {
          const envName = `_closure_${n}_env`;
          const fields = [...capturedSignals.entries()].map(([nm, sym]) => `${sym.ctype} *${nm};`).join(' ');
          const hasGlobalsBefore = ctx.topLevel.some((l: string) => l.trim().length > 0);
          if (hasGlobalsBefore) {
            ctx.topLevel.push(`typedef struct { ${fields} } ${envName};`);
            ctx.topLevel.push(`static ${envName} _closure_${n}_captured;`);
            _envInTopLevel = true;
          } else {
            ctx.typedefs.push(`typedef struct { ${fields} } ${envName};`);
            ctx.typedefs.push(`static ${envName} _closure_${n}_captured;`);
          }
          // Track captured signal refs for fn body emission
          for (const nm of capturedSignals.keys()) {
            ctx._capturedSignalMap.set(nm, `_closure_${n}_captured.${nm}`);
          }
        }

        // Emit fn
        const retType = callee.name === 'effect' ? 'void'
          : (arrow.body.kind === 'Block' ? 'int32_t' : (ctx.inferArrowReturn(arrow) || 'int32_t'));
        const etIdent = ctx.cTypeToIdent(retType);
        const sigType = callee.name === 'computed' ? `Signal_${etIdent}` : null;

        if (callee.name === 'computed') ctx._inComputedFn = true;
        const fnLines: string[] = [];
        ctx.pushScope();
        if (arrow.body.kind === 'Block') ctx.visitBlock(arrow.body, fnLines, 0);
        else { const c = ctx.exprToC(arrow.body, fnLines, 0); fnLines.push(`return ${c};`); }
        ctx.popScope();
        if (callee.name === 'computed') ctx._inComputedFn = false;

        // Clear captured signal map after fn body (back to main context)
        if (capturedSignals.size > 0) {
          for (const nm of capturedSignals.keys()) ctx._capturedSignalMap.delete(nm);
        }

        // No leading blank when env is adjacent in topLevel; add blank otherwise (env in typedefs)
        if (!_envInTopLevel) ctx.topLevel.push('');
        ctx.topLevel.push(`static ${retType} ${fnName}(void) {`);
        for (const l of fnLines) ctx.topLevel.push('    ' + l);
        ctx.topLevel.push('}');
        ctx.topLevel.push('');

        // Emit env init in current scope (main/function body)
        if (capturedSignals.size > 0) {
          const envInit = `(_closure_${n}_env){ ${[...capturedSignals.keys()].map((nm: string) => `.${nm} = &${nm}`).join(', ')} }`;
          lines.push(' '.repeat(ctx.indent * depth) + `_closure_${n}_captured = ${envInit};`);
          // Track persistent captures for batch fns (separate from fn-body map)
          for (const nm of capturedSignals.keys()) {
            ctx._persistentCaptureRefs.set(nm, `_closure_${n}_captured.${nm}`);
          }
        }

        if (callee.name === 'computed') {
          // Signal the VarDecl that the result type is Signal_T (not raw T)
          ctx._lastComputedSigType = sigType!;
          ctx._lastComputedElemType = etIdent;
          return `tsc_computed_${etIdent}(${fnName})`;
        }
        return `tsc_effect(${fnName})`;
      }
    }

    return null;
}

export function _dispatchStdWs(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // WebSocket methods: ws.send(), ws.close(), ws.onMessage(), ws.onClose(), ws.sendBytes()
    if (ctx._stdWsImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _wsSym = ctx.lookup(callee.object.name);
      if (_wsSym?._isWebSocket || _wsSym?.ctype === 'TscWebSocket' || _wsSym?.ctype === 'TscWebSocket *') {
        const _wsIsPtr = _wsSym?.ctype?.endsWith('*');
        const _wsExprC = ctx.exprToC(callee.object, lines, depth);
        const wsRef = _wsIsPtr ? _wsExprC : `&${_wsExprC}`;
        const _wsProp = callee.prop;
        if (_wsProp === 'send') {
          const msgC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
          return `tsc_ws_send(${wsRef}, ${msgC})`;
        }
        if (_wsProp === 'close') {
          return `tsc_ws_close(${wsRef})`;
        }
        if (_wsProp === 'sendBytes') {
          const dataExprC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '(_empty_arr)';
          return `tsc_ws_send_bytes(${wsRef}, ${dataExprC}.data, ${dataExprC}.length)`;
        }
        const _wsHoistCb = (paramTypes: string[] | undefined): string => {
          const cbArg = args[0]?.expr;
          if (cbArg?.kind === 'Arrow') {
            ctx._lambdaParamHint = paramTypes ?? (cbArg.params ?? []).map((p: Param) => p.typeAnn ? ctx.resolveType(p.typeAnn) : 'String');
            const cbName = ctx.hoistArrow(cbArg, 'void');
            ctx._lambdaParamHint = null;
            return cbName;
          }
          return cbArg ? ctx.exprToC(cbArg, lines, depth) : 'NULL';
        };
        if (_wsProp === 'onMessage') return `tsc_ws_on_message(${wsRef}, ${_wsHoistCb(undefined)})`;
        if (_wsProp === 'onClose')   return `tsc_ws_on_close(${wsRef}, ${_wsHoistCb([])})`;
      }
      // WebSocketServer methods: server.onConnect(cb), server.listen(port)
      if (_wsSym?._isWsServer || _wsSym?.ctype === 'TscWebSocketServer') {
        const svrRef = `&${callee.object.name}`;
        const _svrProp = callee.prop;
        if (_svrProp === 'listen') {
          const portC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_ws_server_listen(${svrRef}, ${portC})`;
        }
        if (_svrProp === 'onConnect') {
          const cbArg = args[0]?.expr;
          let cbName;
          if (cbArg?.kind === 'Arrow') {
            const paramNames = (cbArg.params ?? []).map((p: Param, i: number) => p.name ?? `_p${i}`);
            const paramTypes = ['TscWebSocket *'];
            const paramStrs = paramTypes.map((t: string, i: number) => `${t}${paramNames[i] ?? `_p${i}`}`);
            const handlerLines: string[] = [];
            ctx.pushScope();
            for (let i = 0; i < Math.min(paramNames.length, paramTypes.length); i++) {
              ctx.define(paramNames[i], { ctype: paramTypes[i], varKind: 'const', _isWebSocket: true });
            }
            if (cbArg.body.kind === 'Block') ctx.visitBlock(cbArg.body, handlerLines, 0);
            else { const c = ctx.exprToC(cbArg.body, handlerLines, 0); handlerLines.push(`return ${c};`); }
            ctx.popScope();
            const n = ctx._handlerCount ?? 0;
            ctx._handlerCount = n + 1;
            cbName = `_lambda_${n}_void`;
            ctx.topLevel.push(`static void ${cbName}(${paramStrs.join(', ')}) {`);
            for (const l of handlerLines) ctx.topLevel.push('    ' + l);
            ctx.topLevel.push('}');
            ctx.topLevel.push('');
          } else {
            cbName = cbArg ? ctx.exprToC(cbArg, lines, depth) : 'NULL';
          }
          return `tsc_ws_server_on_connect(${svrRef}, ${cbName})`;
        }
      }
    }

    return null;
}

export function _dispatchStdNet(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // std/net: net.listen(port, handler) / net.connect handled via async
    if (ctx._stdNetImported && callee.kind === 'Member' &&
        callee.object.kind === 'Ident' && callee.object.name === 'net') {
      const _netProp = callee.prop;
      if (_netProp === 'listen') {
        const portC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
        const cbArg = args[1]?.expr;
        const n = ctx._handlerCount ?? 0;
        ctx._handlerCount = n + 1;
        const handlerName = `_handler_${n}`;
        if (cbArg?.kind === 'Arrow') {
          const paramTypes = ['TscSocket *'];
            const paramNames = (cbArg.params ?? []).map((p: { name?: string }, i: number) => p.name ?? `_p${i}`);
            const paramStrs = paramTypes.map((t: string, i: number) => `${t}${paramNames[i] ?? `_p${i}`}`);
            const handlerLines: string[] = [];
          ctx.pushScope();
          for (let i = 0; i < paramNames.length; i++) {
            ctx.define(paramNames[i], { ctype: paramTypes[i], varKind: 'const' });
          }
          if (cbArg.body.kind === 'Block') ctx.visitBlock(cbArg.body, handlerLines, 0);
          else { const c = ctx.exprToC(cbArg.body, handlerLines, 0); handlerLines.push(`return ${c};`); }
          ctx.popScope();
          ctx.topLevel.push(`static void ${handlerName}(${paramStrs.join(', ')}) {`);
          for (const l of handlerLines) ctx.topLevel.push('    ' + l);
          ctx.topLevel.push('}');
          ctx.topLevel.push('');
        }
        return `tsc_net_listen(${portC}, ${handlerName})`;
      }
    }

    // std/net: HttpServer methods (server.get, server.post, server.listen)
    if (ctx._stdNetImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _serverSym = ctx.lookup(callee.object.name);
      if (_serverSym?._isHttpServer) {
        const svrRef = `&${callee.object.name}`;
        const _svrProp = callee.prop;
        if (_svrProp === 'listen') return `tsc_http_server_listen(${svrRef})`;
        if (_svrProp === 'get' || _svrProp === 'post') {
          const pathC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("/")';
          const cbArg = args[1]?.expr;
          const n = ctx._handlerCount ?? 0;
          ctx._handlerCount = n + 1;
          const handlerName = `_handler_${n}`;
          if (cbArg?.kind === 'Arrow') {
            const paramNames = (cbArg.params ?? []).map((p: Param, i: number) => p.name ?? `_p${i}`);
            const paramTypes = ['TscRequest *', 'TscResponse *'];
            const paramStrs = paramTypes.map((t: string, i: number) => `${t}${paramNames[i] ?? `_p${i}`}`);
            const handlerLines: string[] = [];
            const savedInFunc = ctx.inFunction;
            const savedStackLen = ctx._blockCleanupStack.length;
            ctx.inFunction = true;
            ctx._blockCleanupStack.push({ list: [], set: new Set() });
            ctx.pushScope();
            for (let i = 0; i < Math.min(paramNames.length, paramTypes.length); i++) {
              ctx.define(paramNames[i], { ctype: paramTypes[i], varKind: 'const', _isNetParam: true });
            }
            if (cbArg.body.kind === 'Block') ctx.visitBlock(cbArg.body, handlerLines, 0);
            else { const c = ctx.exprToC(cbArg.body, handlerLines, 0); handlerLines.push(`return ${c};`); }
            const handlerCleanup = ctx._blockCleanupStack.pop();
            for (let i = handlerCleanup!.list.length - 1; i >= 0; i--) handlerLines.push(handlerCleanup!.list[i] + ';');
            ctx.popScope();
            ctx.inFunction = savedInFunc;
            while (ctx._blockCleanupStack.length > savedStackLen) ctx._blockCleanupStack.pop();
            ctx.topLevel.push(`static void ${handlerName}(${paramStrs.join(', ')}) {`);
            for (const l of handlerLines) ctx.topLevel.push('    ' + l);
            ctx.topLevel.push('}');
            ctx.topLevel.push('');
          }
          return `tsc_http_server_${_svrProp}(${svrRef}, ${pathC}, ${handlerName})`;
        }
      }
      // TscResponse methods: res.text, res.json
      if (_serverSym?.ctype === 'TscResponse *') {
        const resC = callee.object.name;
        if (callee.prop === 'text') {
          const textC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
          return `tsc_response_text(${resC}, ${textC})`;
        }
        if (callee.prop === 'json') {
          const jsonC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
          return `tsc_response_json(${resC}, ${jsonC})`;
        }
      }
      // TscSocket methods: sock.close()
      if (_serverSym?.ctype === 'TscSocket *' || _serverSym?.ctype === 'TscSocket') {
        if (callee.prop === 'close') {
          const sockC = ctx.exprToC(callee.object, lines, depth);
          const sockRef = _serverSym.ctype.endsWith('*') ? sockC : `&${sockC}`;
          return `tsc_socket_close(${sockRef})`;
        }
      }
    }

    return null;
}

export function _dispatchStdFs(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // fs namespace: fs.watch(), fs.readFileSync(), fs.writeFileSync(), etc.
    if (ctx._stdFsImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _fsSym = ctx.lookup(callee.object.name);
      if (_fsSym?._isFsNamespace) {
        const _fsProp = callee.prop;
        const _a0 = () => args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
        const _a1 = () => args[1] ? ctx.exprToC(args[1].expr, lines, depth) : 'STR_LIT("")';
        if (_fsProp === 'watch') {
          const pathC = _a0();
          const cbArg = args[1]?.expr;
          let cbName;
          if (cbArg?.kind === 'Arrow') {
            ctx._lambdaParamHint = (cbArg.params ?? []).map((p: Param) => p.typeAnn ? ctx.resolveType(p.typeAnn) : 'String');
            cbName = ctx.hoistArrow(cbArg, 'void');
            ctx._lambdaParamHint = null;
          } else {
            cbName = cbArg ? ctx.exprToC(cbArg, lines, depth) : 'NULL';
          }
          return `tsc_fs_watch(${pathC}, ${cbName})`;
        }
        if (_fsProp === 'readFileSync')      return `tsc_fs_read_sync(${_a0()})`;
        if (_fsProp === 'readFileBytesSync') return `tsc_fs_read_bytes_sync(${_a0()})`;
        if (_fsProp === 'writeFileSync')     return `tsc_fs_write_sync(${_a0()}, ${_a1()})`;
        if (_fsProp === 'appendFileSync')    return `tsc_fs_append_sync(${_a0()}, ${_a1()})`;
        if (_fsProp === 'existsSync')        return `tsc_fs_exists_sync(${_a0()})`;
        if (_fsProp === 'mkdirSync')         return `tsc_fs_mkdir_sync(${_a0()})`;
        if (_fsProp === 'readDirSync')        return `tsc_fs_readdir_sync(${_a0()})`;
        if (_fsProp === 'removeSync')        return `tsc_fs_remove_sync(${_a0()})`;
        if (_fsProp === 'renameSync')        return `tsc_fs_rename_sync(${_a0()}, ${_a1()})`;
        if (_fsProp === 'statSync')          return `tsc_fs_stat_sync(${_a0()})`;
      }
    }

    return null;
}

export function _dispatchStdTemporal(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // Temporal static methods: PlainDate.from(), Instant.now(), etc.
    if (ctx._stdTemporalImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _tClass = callee.object.name;
      const _tProp  = callee.prop;
      const _temporalSuppressConst = () => { ctx._lastSuppressConst = true; };
      if (_tClass === 'PlainDate' && _tProp === 'from') {
        const y = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
        const m = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : '0';
        const d = args[2] ? ctx.exprToC(args[2].expr, lines, depth) : '0';
        _temporalSuppressConst();
        return `tsc_plain_date_from(${y}, ${m}, ${d})`;
      }
      if (_tClass === 'PlainTime' && _tProp === 'from') {
        const h = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
        const m = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : '0';
        const s = args[2] ? ctx.exprToC(args[2].expr, lines, depth) : '0';
        _temporalSuppressConst();
        return `tsc_plain_time_from(${h}, ${m}, ${s})`;
      }
      if (_tClass === 'PlainDateTime' && _tProp === 'from') {
        const date = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '(TscPlainDate){0}';
        const time = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : '(TscPlainTime){0}';
        _temporalSuppressConst();
        return `tsc_plain_datetime_from(${date}, ${time})`;
      }
      if (_tClass === 'Instant' && _tProp === 'now') { _temporalSuppressConst(); return `tsc_instant_now()`; }
      if (_tClass === 'ZonedDateTime' && _tProp === 'now') {
        const tz = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("UTC")';
        _temporalSuppressConst();
        return `tsc_zoned_datetime_now(${tz})`;
      }
      if (_tClass === 'Now') {
        if (_tProp === 'instant') { _temporalSuppressConst(); return `tsc_instant_now()`; }
        if (_tProp === 'plainDate') {
          const tz = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("UTC")';
          _temporalSuppressConst();
          return `tsc_now_plain_date(${tz})`;
        }
      }
      if (_tClass === 'Duration' && _tProp === 'from') {
        const objArg = args[0]?.expr;
        if (objArg?.kind === 'ObjLit') {
          const props: Record<string, string> = {};
          for (const p of (objArg.props ?? [])) {
            props[String(p.key)] = p.value ? ctx.exprToC(p.value, lines, depth) : '0';
          }
          _temporalSuppressConst();
          if ('days' in props && !('hours' in props) && !('minutes' in props)) {
            return `tsc_duration_from_days(${props.days ?? '0'})`;
          }
          const h = props.hours ?? '0';
          const m = props.minutes ?? '0';
          const s = props.seconds ?? '0';
          return `tsc_duration_from_hms(${h}, ${m}, ${s})`;
        }
      }
    }

    // Temporal instance methods: d.add(), d.until(), etc.
    if (ctx._stdTemporalImported && callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _tSym = ctx.lookup(callee.object.name);
      const _tCtype = _tSym?.ctype ?? '';
      if (_tCtype === 'TscPlainDate') {
        const _tName = callee.object.name;
        const _tProp = callee.prop;
        if (_tProp === 'add') {
          ctx._lastSuppressConst = true;
          const durC = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '(TscDuration){0}';
          return `tsc_plain_date_add(${_tName}, ${durC})`;
        }
        if (_tProp === 'until') {
          ctx._lastSuppressConst = true;
          const d2C = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '(TscPlainDate){0}';
          return `tsc_plain_date_until(${_tName}, ${d2C})`;
        }
      }
    }

    return null;
}

export function _dispatchStdBuffer(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // Buffer method calls: buf.fill(), buf.slice()
    if (callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _bufSym = ctx.lookup(callee.object.name);
      if (_bufSym?._isBuffer || _bufSym?.ctype === 'Buffer') {
        const _bufName = callee.object.name;
        const _bufProp = callee.prop;
        if (_bufProp === 'fill') {
          const val = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `memset(${_bufName}.data, ${val}, ${_bufName}.length)`;
        }
        if (_bufProp === 'slice') {
          const start = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          const end = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : `${_bufName}.length`;
          const lenExpr = (args[0]?.expr?.kind === 'Literal' && args[1]?.expr?.kind === 'Literal')
            ? String(parseFloat(args[1].expr.value) - parseFloat(args[0].expr.value))
            : `(${end}) - (${start})`;
          return `{.data = ${_bufName}.data + ${start}, .length = ${lenExpr}}`;
        }
      }
    }

    return null;
}

export function _dispatchStdDataView(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    if (callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _dvSym = ctx.lookup(callee.object.name);
      if (_dvSym?._isDataView || _dvSym?.ctype === 'DataView') {
        const _dvName = callee.object.name;
        const _dvProp = callee.prop;
        const base = `${_dvName}.data + ${_dvName}.byte_offset`;
        const I = ' '.repeat(ctx.indent * depth);

        // Backward compat: *LE aliases
        const leAliases: Record<string, [string, string, boolean]> = {
          getU16LE: ['get', 'U16', true], setU16LE: ['set', 'U16', true],
          getU32LE: ['get', 'U32', true], setU32LE: ['set', 'U32', true],
          getF64LE: ['get', 'F64', true], setF64LE: ['set', 'F64', true],
        };
        if (leAliases[_dvProp]) {
          const [dir, type, le] = leAliases[_dvProp];
          const fakeProp = dir + type;
          const fakeArgs = dir === 'get' ? args : args;
          return ctx._dvOp(_dvName, base, I, dir, type, le, fakeArgs, lines, depth, _dvSym);
        }

        // Standard methods: getU8, getI8, getU16, getI16, ..., setU8, setI8, ...
        const dvMethods: Record<string, [string, string, number]> = {
          getU8:['get','U8',1], getI8:['get','I8',1],
          getU16:['get','U16',2], getI16:['get','I16',2],
          getU32:['get','U32',4], getI32:['get','I32',4],
          getU64:['get','U64',8], getI64:['get','I64',8],
          getF32:['get','F32',4], getF64:['get','F64',8],
          setU8:['set','U8',1], setI8:['set','I8',1],
          setU16:['set','U16',2], setI16:['set','I16',2],
          setU32:['set','U32',4], setI32:['set','I32',4],
          setU64:['set','U64',8], setI64:['set','I64',8],
          setF32:['set','F32',4], setF64:['set','F64',8],
        };

        if (_dvProp === 'byteLength') return `(size_t)${_dvName}.byte_length`;
        if (_dvProp === 'byteOffset') return `(size_t)${_dvName}.byte_offset`;

        const m = dvMethods[_dvProp];
        if (m) {
          const [dir, type, sz] = m;
          const leArgIdx = dir === 'get' ? 1 : 2;
          const leNode = args[leArgIdx]?.expr;
          const le = leNode ? (leNode.kind === 'Literal' && leNode.value === 'true') : false;
          return ctx._dvOp(_dvName, base, I, dir, type, le, args, lines, depth, _dvSym);
        }
      }
    }

    return null;
}

export function _dvOp(ctx: CodeGenContext, _dvName: string, base: string, I: string, dir: string, type: string, le: boolean, args: Argument[], lines: string[], depth: number, _dvSym: SymbolInfo) {
    const _dvIdx = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
    const ptr = `(${base} + ${_dvIdx})`;
    const sz = ({ U8:1, I8:1, U16:2, I16:2, U32:4, I32:4, U64:8, I64:8, F32:4, F64:8 } as Record<string, number>)[type];
    const cType = ({ U8:'uint8_t', I8:'int8_t', U16:'uint16_t', I16:'int16_t', U32:'uint32_t', I32:'int32_t', U64:'uint64_t', I64:'int64_t', F32:'float', F64:'double' } as Record<string, string>)[type];
    const castCType = ({ U8:'uint8_t', I8:'int8_t', U16:'uint16_t', I16:'int16_t', U32:'uint32_t', I32:'int32_t', U64:'uint64_t', I64:'int64_t', F32:'float', F64:'double' } as Record<string, string>)[type];

    // Bounds check
    if (_dvSym?._dvCap != null) {
      const offsetNode = args[0]?.expr;
      const offsetVal = (offsetNode?.kind === 'Literal' && offsetNode?.litType === 'number') ? parseInt(offsetNode.value) : null;
      if (offsetVal != null && offsetVal + sz > _dvSym._dvCap) {
        throw ctx.errorCode('E418', null, { detail: `TypeError: DataView.${dir}${type} at offset ${offsetVal} requires ${sz} bytes, but buffer length is ${_dvSym._dvCap}` });
      }
    }

    if (sz === 1) {
      if (dir === 'set') {
        const val = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : '0';
        return `${ptr}[0] = (${cType})${val}`;
      }
      return `(${castCType})${ptr}[0]`;
    }

    // Multi-byte
    const n = ctx._dvTmpCount = (ctx._dvTmpCount ?? 0) + 1;
    const tmp = `_dv_${n}`;

    if (dir === 'get') {
      if (le) {
        lines.push(`${I}${cType} ${tmp} = {0}; memcpy(&${tmp}, ${ptr}, ${sz});`);
      } else {
        lines.push(`${I}${cType} ${tmp} = {0};`);
        lines.push(`${I}uint8_t *_p = (uint8_t*)&${tmp};`);
        lines.push(`${I}uint8_t *_s = ${ptr};`);
        if (sz === 2) {
          lines.push(`${I}_p[1] = _s[0]; _p[0] = _s[1];`);
        } else if (sz === 4) {
          lines.push(`${I}_p[3] = _s[0]; _p[2] = _s[1]; _p[1] = _s[2]; _p[0] = _s[3];`);
        } else if (sz === 8) {
          lines.push(`${I}_p[7] = _s[0]; _p[6] = _s[1]; _p[5] = _s[2]; _p[4] = _s[3]; _p[3] = _s[4]; _p[2] = _s[5]; _p[1] = _s[6]; _p[0] = _s[7];`);
        }
      }
      return `(${castCType})${tmp}`;
    }

    // set
    const val = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : '0';
    if (le) {
      lines.push(`${I}${cType} ${tmp} = (${cType})${val};`);
      lines.push(`${I}memcpy((void*)${ptr}, &${tmp}, ${sz});`);
    } else {
      lines.push(`${I}${cType} ${tmp} = (${cType})${val};`);
      lines.push(`${I}uint8_t *_p = (uint8_t*)&${tmp};`);
      lines.push(`${I}uint8_t *_s = (uint8_t*)${ptr};`);
      if (sz === 2) {
        lines.push(`${I}_s[0] = _p[1]; _s[1] = _p[0];`);
      } else if (sz === 4) {
        lines.push(`${I}_s[0] = _p[3]; _s[1] = _p[2]; _s[2] = _p[1]; _s[3] = _p[0];`);
      } else if (sz === 8) {
        lines.push(`${I}_s[0] = _p[7]; _s[1] = _p[6]; _s[2] = _p[5]; _s[3] = _p[4]; _s[4] = _p[3]; _s[5] = _p[2]; _s[6] = _p[1]; _s[7] = _p[0];`);
      }
    }
    return `(void)0`;
}

export function _dispatchStdHashMap(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // HashMap method calls: m.set(), m.get(), m.has(), m.delete()
    if (callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _hmSym = ctx.lookup(callee.object.name);
      if (_hmSym?._isHashMap) {
        const _hmName = callee.object.name;
        const _hmProp = callee.prop;
        const _hmSfx = _hmSym._hmSuffix;
        const _hmArg0 = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
        if (_hmProp === 'has') return `tsc_hashmap_has_${_hmSfx}(&${_hmName}, ${_hmArg0})`;
        if (_hmProp === 'set') {
          // Compile-time capacity overflow check
          if (_hmSym._hmCap > 0) {
            _hmSym._hmSetCount = (_hmSym._hmSetCount ?? 0) + 1;
            if (_hmSym._hmSetCount > _hmSym._hmCap) {
              throw ctx.errorCode('E418', null, { detail: `RuntimeError: HashMap capacity exceeded: max ${_hmSym._hmCap}, attempted to insert ${_hmSym._hmSetCount}th entry` });
            }
          }
          const _hmArg1 = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : '0';
          return `tsc_hashmap_set_${_hmSfx}(&${_hmName}, ${_hmArg0}, ${_hmArg1})`;
        }
        if (_hmProp === 'get') {
          const vIdent = ctx.cTypeToIdent(_hmSym._hmValType ?? 'int32_t');
          const optName = `opt_${vIdent}`;
          ctx._ensureOptStruct(optName, _hmSym._hmValType ?? 'int32_t');
          ctx._lastSuppressConst = true;
          return `tsc_hashmap_get_${_hmSfx}(&${_hmName}, ${_hmArg0})`;
        }
        if (_hmProp === 'delete') return `tsc_hashmap_delete_${_hmSfx}(&${_hmName}, ${_hmArg0})`;
      }
    }

    return null;
}

export function _dispatchStdSet(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // Set method calls: s.add(), s.has(), s.delete(), s.clear()
    if (callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _setSym = ctx.lookup(callee.object.name);
      if (_setSym?._isSet) {
        const _sName = callee.object.name;
        const _sProp = callee.prop;
        const _sSfx  = _setSym._setSuffix;
        const _sArg0 = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
        if (_sProp === 'add')    return `tsc_set_add_${_sSfx}(&${_sName}, ${_sArg0})`;
        if (_sProp === 'has')    return `tsc_set_has_${_sSfx}(&${_sName}, ${_sArg0})`;
        if (_sProp === 'delete') {
          ctx._lastSuppressConst = true;
          return `tsc_set_delete_${_sSfx}(&${_sName}, ${_sArg0})`;
        }
        if (_sProp === 'clear')  return `tsc_set_clear_${_sSfx}(&${_sName})`;
        if (_sProp === 'forEach') {
          const cbArg = args[0]?.expr;
          if (!cbArg || cbArg.kind !== 'Arrow') return null;
          const _sElemCType = _setSym._setElemCType ?? 'int32_t';
          ctx._lambdaParamHint = [_sElemCType];
          const cbName = ctx.hoistArrow(cbArg, 'void');
          ctx._lambdaParamHint = null;
          return `tsc_set_for_each_${_sSfx}(&${_sName}, ${cbName})`;
        }
        if (_sProp === 'values' || _sProp === 'keys') {
          const _sElemCType = _setSym._setElemCType ?? 'int32_t';
          const _sElemIdent = ctx.cTypeToIdent(_sElemCType);
          ctx._ensureArrayStruct(`Array_${_sElemIdent}`, _sElemCType);
          return `tsc_set_values_${_sSfx}(${_sName})`;
        }
        if (_sProp === 'entries') {
          const _sElemCType = _setSym._setElemCType ?? 'int32_t';
          const _sElemIdent = ctx.cTypeToIdent(_sElemCType);
          const tupleName = `Tuple_${_sElemIdent}_${_sElemIdent}`;
          ctx.addTop(`typedef struct { ${_sElemCType} _0; ${_sElemCType} _1; } ${tupleName};`);
          ctx._ensureArrayStruct(`Array_${tupleName}`, tupleName);
          return `tsc_set_entries_${_sSfx}(${_sName})`;
        }
        if (_sProp === 'union') {
          const other = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_set_union_${_sSfx}(${_sName}, ${other})`;
        }
        if (_sProp === 'intersection') {
          const other = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_set_intersection_${_sSfx}(${_sName}, ${other})`;
        }
        if (_sProp === 'difference') {
          const other = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_set_difference_${_sSfx}(${_sName}, ${other})`;
        }
        if (_sProp === 'symmetricDifference') {
          const other = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_set_symmetric_difference_${_sSfx}(${_sName}, ${other})`;
        }
        if (_sProp === 'isSubsetOf') {
          const other = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_set_is_subset_of_${_sSfx}(${_sName}, ${other})`;
        }
        if (_sProp === 'isSupersetOf') {
          const other = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_set_is_superset_of_${_sSfx}(${_sName}, ${other})`;
        }
        if (_sProp === 'isDisjointFrom') {
          const other = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : '0';
          return `tsc_set_is_disjoint_from_${_sSfx}(${_sName}, ${other})`;
        }
      }
    }

    return null;
}

export function _dispatchStdTasks(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // Tasks method calls: tasks.add(), tasks.run(), tasks.stop()
    if (callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _tasksSym = ctx.lookup(callee.object.name);
      if (_tasksSym?._isTasks) {
        const _tasksName = callee.object.name;
        const _tasksProp = callee.prop;
        if (_tasksProp === 'run') return `tsc_tasks_run(&${_tasksName})`;
        if (_tasksProp === 'stop') {
          const _label = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
          return `tsc_tasks_stop(&${_tasksName}, ${_label})`;
        }
        if (_tasksProp === 'add') {
          const _label = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
          const _fnName = args[1]?.expr?.kind === 'Ident' ? args[1].expr.name : null;
          const _genInfo = _fnName ? ctx._generatorFuncs?.get(_fnName) : null;
          if (_genInfo) {
            const { stateType, nextFn } = _genInfo;
            const pollFn = `${_fnName}_poll`;
            if (!ctx._emittedTasksPolls.has(pollFn)) {
              ctx._emittedTasksPolls.add(pollFn);
              ctx.topLevel.push('');
              ctx.topLevel.push(`static void ${pollFn}(void *state) {`);
              ctx.topLevel.push(`    ${nextFn}((${stateType} *)state);`);
              ctx.topLevel.push(`}`);
            }
            const stateVar = `_${_fnName}_state_${ctx._tasksStateCount++}`;
            const I = ' '.repeat(ctx.indent * depth);
            lines.push(`${I}static ${stateType} ${stateVar} = {0};`);
            return `tsc_tasks_add(&${_tasksName}, ${_label}, ${pollFn}, &${stateVar})`;
          }
        }
      }
    }

    return null;
}

export function _dispatchStdRegex(ctx: CodeGenContext, node: Call, lines: string[], depth: number) {
    const { callee, args } = node;
    // TscRegex method calls: r.test(), r.match(), r.replace(), r.replaceAll()
    if (callee.kind === 'Member' && callee.object.kind === 'Ident') {
      const _rxSym = ctx.lookup(callee.object.name);
      if (_rxSym?._isRegex) {
        const _rxName = callee.object.name;
        const _rxProp = callee.prop;
        const _rxArg0 = args[0] ? ctx.exprToC(args[0].expr, lines, depth) : 'STR_LIT("")';
        if (_rxProp === 'test') return `tsc_regex_test(&${_rxName}, ${_rxArg0})`;
        if (_rxProp === 'match') {
          ctx._lastSuppressConst = true;
          ctx._ensureArrayStruct('Array_string', 'String');
          ctx._ensureOptStruct('opt_Array_string', 'Array_string');
          return `tsc_regex_match(&${_rxName}, ${_rxArg0})`;
        }
        if (_rxProp === 'replace') {
          ctx._lastSuppressConst = true;
          const _rxArg1 = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : 'STR_LIT("")';
          return `tsc_regex_replace(&${_rxName}, ${_rxArg0}, ${_rxArg1})`;
        }
        if (_rxProp === 'replaceAll') {
          ctx._lastSuppressConst = true;
          const _rxArg1 = args[1] ? ctx.exprToC(args[1].expr, lines, depth) : 'STR_LIT("")';
          return `tsc_regex_replace_all(&${_rxName}, ${_rxArg0}, ${_rxArg1})`;
        }
      }
    }

    return null;
}
