...
> 2026-06-10: `@embedded.stack` → `@stack` + макросы из `import { push, pop, empty } from "std/stack"`: (1) rename декоратора `@embedded.stack` → `@stack` в func.js, (2) добавлен модуль `std/stack` в stdlib-registry.js с `_handleStdStack`, (3) удалён AST node `EmbeddedMacro` целиком (parser.js: standalone statement + expression parsing, codegen/expr/dispatch.js: case), (4) обработка `push`/`pop`/`empty` добавлена в call-dispatch.js (inline C codegen) + infer.js (return types: void/bool/T), (5) тест `phase7/embedded-stack` → `phase7/stack-macro` с новым синтаксисом `import { push, pop, empty } from "std/stack"`, (6) spec обновлён. **Статус: все тесты проходят ✓**
> 2026-06-10: **spec — class allocation strategy** — `07-classes-ownership.md`: расширена таблица Desktop vs Embedded → единая Allocation Strategy (class/@struct/@pool/@heap), добавлены секции `@struct`, `@pool(N)`, `@heap` (future), `@static class field`, `@readonly`. `13-build.md`: подробное описание @struct/@pool заменено на cross-reference. `13-platform-capabilities.md`: reconcile allocator (классы не используют malloc, malloc только для контейнеров). `SPEC.md`: убраны устаревшие имена (`@embedded.inline`, `@embedded.pool`, `Cls.alloc()`). **Статус: 1902/1902 ✓**
> 2026-06-10: **spec — @static function contexts + @inline + built-in decorators index** — `06-functions.md`: добавлена секция "Модификаторы функций" с `@static function` и `@inline function` (подробно: syntax, C-output, use cases, cross-references). `10-async.md`: добавлен полный подраздел `@static async function*` (cooperative scheduler, BSS instance, allocator restrictions). `15-decorators.md`: добавлена секция "Встроенные декораторы" — индекс всех 15 встроенных декораторов с описанием и ссылками на полные спецификации (`@static` × 5 контекстов, `@struct`, `@pool`, `@heap`, `@inline`, `@readonly`, `@packed`, `@align`, `@platform`, `@isr`, `@stack`). **Статус: 1902/1902 ✓**
> 2026-06-10: **spec — clean up @embedded.* and rewrite 15-decorators-overview** — `11-concurrency.md`: удалена секция `### @struct` (forced inline для функций) — перенесена в `06-functions.md` как `@inline`; обновлена сводная таблица аннотаций (`@struct` → `@inline`). `15-decorators-overview.md`: переписан из битого stub (6 строк mojibake) в полноценный обзор built-in decorators на классах/полях/функциях/переменных с таблицами и ссылками. **Статус: 1902/1902 ✓**
> 2026-06-10: **test: pool on desktop (10 new tests)** — `phase11/pool/desktop-pool-ok` (`@pool` на desktop, fragment), `desktop-pool-alloc` (multiple `new PoolClass()`), `desktop-pool-move` (use-after-move, **failing** — bug fix), `desktop-pool-allocator-heap` (pool + heap), `desktop-pool-allocator-static` (pool + static), `embedded-pool-regression` (регрессия, passing), `desktop-pool-ctor` (pool с конструктором), `desktop-pool-method` (методы на pool, **failing** — bug fix, expected.c фиксирует текущий невалидный вывод), `desktop-pool-throws` (new в throws), `desktop-pool-trycatch` (try/catch). Текущий статус: 1 failing (`desktop-pool-move` — use-after-move detection), 9 passing. **Статус: 1908/1909 ✓**
> 2026-06-10: **fix: remove @pool warning on non-embedded platform** — `class.js:49-54`: убран `poolDec && !isEmbedded` warning (pool теперь работает на desktop). Также убран `isEmbedded` check в `class.js:271` (`_emitPoolClass`) — pool эмиссия теперь работает на всех платформах. Удалён устаревший тест `err-non-embedded` (тест проверял что pool warning = error, теперь pool на desktop = OK по решению спеки). **Статус: 1908/1909 ✓**
> 2026-06-10: **fix: pool methods dispatch** — `method-dispatch.js:690-706`: добавлена обработка `opt_ref_*` pool refs перед основной проверкой — методы на pool instance генерируются как `ClassName_method(c.value, args)` (pointer-based). `infer.js:351-353`: strip `opt_ref_` prefix для pool при lookup класса в `_inferMemberCall`. Тест `desktop-pool-method` теперь passing (c.increment() → Counter_increment(c.value)). **Статус: 1908/1909 ✓**
> 2026-06-10: **fix: pool mask overflow при N>16** — `class.js:_emitPoolClass` расширил tier маски: `uint8_t` (1-8), `uint16_t` (9-16), `uint32_t` (17-32), `uint64_t` (33-64). N > 64 → compile error. Все `1 << _i` заменены на `(maskType)1 << _i` (избегает UB на AVR с 16-bit int при N > 16). Также `maskType` теперь деструктуризуется из cls в `_ensurePoolAlloc`/`_ensurePoolDrop`. Новые тесты: `pool-32` (uint32_t tier), `pool-64` (uint64_t tier), `err-pool-65` (compile error). Обновлены expected.c для 18 существующих pool тестов (новый формат маски/сдвига). **Статус: 1844/1845 ✓**
> 2026-06-10: **fix: use-after-move detection gaps (narrowing + assignment)** — `expr/dispatch.js:34`: добавлен `this._checkMoved(sym2, node, node.name)` в narrowing path для `opt_` типов (теперь pool refs внутри `if (x != null)` тоже проверяются). `expr/assign.js`: добавлен move tracking для `b = a` где RHS = Ident of `opt_ref_` type — `_moved` устанавливается, `_movedLine` + `_movedSourceNode` сохраняются, при `let` генерируется `a = (opt_ref_T){false, NULL, -1}` (zero-out). Проверка двойного move (E002) и const move (E003). Тест `desktop-pool-move` теперь passing. **Статус: 1845/1845 ✓**
> 2026-06-10: **Plan v9 complete — full spec + gap analysis + desktop pool** — Итог плана v9 (9 коммитов): (1-3) обновлена спека — class allocation strategy (`@struct`/`@pool`/`@heap`), `@static` контексты (class field, function, generator, async gen), `@inline` (бывший `@struct` для функций), встроенные декораторы индекс (15 штук), `15-decorators-overview.md` переписан; (4) 10 desktop pool тестов (alloc, move, ctor, method, throws, trycatch + heap/static allocator variants + embedded regression); (5) `@pool` warning убран, pool эмиссия работает на всех платформах; (6) pool methods dispatch починен (`opt_ref_*` prefix handling); (7) pool mask overflow починен (uint8/16/32/64 tiers, N > 64 → compile error, `(maskType)1 << _i`); (8) use-after-move detection починен (narrowing bypass + assignment tracking). **Финальный статус: 1845/1845 ✓**
> 2026-06-10: **doc/ cleanup** — удалена вся директория `doc/` кроме `doc/ru/index.md` и `doc/ru/plan.md` (368 файлов удалено). Удалены 9 локализаций (de, en, es, fr, ja, pt-br, tr, zh-cn, zh-tw) + все поддиректории `doc/ru/` (01-intro, 02-syntax, 03-types, 04-classes, 05-memory, 06-errors, 07-concurrency, 08-modules, 09-build, 10-stdlib, 11-compiler, 12-migration). Документация устарела (содержала `@embedded.inline`, `@embedded.pool`, `Cls.alloc()` и другие переименованные API), будет переписана заново из `spec/` для готовящейся книги. **`spec/` остаётся единственным источником истины.** Регрессия: 1845/1845 ✓ (не затронуто).
> 2026-06-10: **cleanup: remove artifacts** — `.gitignore`: добавлен `.tsclang-tmp2/`. Удалены: `.tsclang-tmp2/` (артефакты сборки: CMakeLists.txt, input.c, test_move2.c), `_debug.mjs` (debug-скрипт с hardcoded test case), `test/cases/11/ram-stack/err-stack-callchain-branch/` (неполный черновик — только `expected.error`, полная версия уже есть в `test/cases/phase11/ram-stack/`), `input.c` + `test.c` (root, попали под `*.c` в .gitignore). Регрессия: 1845/1845 ✓ (не затронуто).
> 2026-06-10: **test: @heap heap-allocated classes (8 new tests, all failing)** — `test/cases/phase11/heap/`: `desktop-heap-ok` (простой heap class), `err-heap-allocator-static` (`@heap` + static = compile error), `desktop-heap-null` (`Node | null` = pointer), `desktop-heap-move` (use-after-move, failing по дизайну), `desktop-heap-recursive` (дерево/список), `desktop-heap-method` (метод на heap), `desktop-heap-auto-drop` (auto-free at scope), `desktop-heap-trycatch` (heap в try/catch). Текущий статус: 2 failing (move, err-heap-allocator-static) + 6 skip (до реализации нет expected.c). **Статус: 61/63 ✓**
> 2026-06-10: **feat: @heap decorator parsing + type system** — `class.js:visitClassDecl`: распознан `@heap` декоратор. Валидация: `@heap` + `allocator: "static"` → compile error; `@heap` + `@pool`/`@struct` → mutually exclusive; `@heap` + inheritance → error. `_classEntry._isHeap: true` маркер. Новые методы: `_markHeapClass`, `_ensureHeapDestructor` (auto-gen string release для string полей). `types/infer.js:204`: `new X()` для `@heap` → `X*`. `types/resolve.js:177-179`: `X | null` = `X*`. Тест `err-heap-allocator-static` теперь passing. Регрессия: 62/63 ✓ (только `desktop-heap-move` failing по дизайну).
> 2026-06-10: **feat: new X() codegen для @heap** — `misc/new-expr.js`: `new X()` для `@heap` классов генерирует `X *p = (X *)tsc_malloc(sizeof(X)); *p = X_new(args);`. `#include <stdlib.h>` добавляется автоматически. Recursive types работают: `Node { next: Node | null }` → `Node *next` (размер конечен). Тесты: `desktop-heap-ok`, `desktop-heap-null`, `desktop-heap-recursive`, `desktop-heap-auto-drop`, `desktop-heap-method`, `desktop-heap-trycatch` — passing (с placeholder expected.c, методы будут исправлены в коммите 4). `desktop-heap-move` failing (move semantics + use-after-move detection ещё не реализованы для heap). Регрессия: 68/69 ✓ (8 heap тестов + существующие 60).
> 2026-06-10: **feat: heap field access + method dispatch + move semantics** — `expr/dispatch.js:198`: heap pointer var `p.field` → `p->field`. `codegen.js:define`: auto-mark `_isHeap: true` на символах с ctype `ClassName *` где класс — `@heap`. `calls/method-dispatch.js:691`: heap instance `c.method()` → `ClassName_method(c, args)` (без `&`, т.к. c уже pointer). `expr/assign.js:147`: move tracking для `b = a` где a — heap pointer — `a = NULL` (zero-out), `_moved` отслеживается. Тесты: `desktop-heap-method` теперь passing (Counter_increment(c) корректно), `desktop-heap-null`/`desktop-heap-recursive` правильно используют `->`. `desktop-heap-move` failing (коммит 5 нужен для auto-drop). Регрессия: 1852/1853 ✓.
> 2026-06-10: **feat: heap auto-drop + cleanup** — `codegen.js:_emitHeapDrops` + `stmt.js:visitBlock` (parallel к pool): heap vars регистрируются через `define` хук (`_currentBlockHeapVars.push`), drop-ятся в конце block: `if (p != NULL) { X_destructor(p); tsc_free(p); }`. `_ensureHeapDestructor` (auto-gen string release для string полей). `vardecl.js`: move tracking для heap перемещён в правильное место (после opt_ блока). Все 8 heap тестов passing, включая `desktop-heap-move` (`use of moved value: "a"` корректно). Регрессия: 1853/1853 ✓.
> 2026-06-10: **docs: @heap implemented (not future) + final regression** — `spec/07-classes-ownership.md`: обновлена секция `@heap` — убрано "future feature", статус "Реализовано (Шаг 2, коммит cc97b2e)". Добавлены C-вывод (включая `_destructor` auto-gen), move semantics с E002 detection, ограничения (без inheritance, без `@pool`/`@struct` совместимости). Allocation Strategy таблица: `@heap (future)` → `@heap` (без future). Принципы: 1) `@heap` decorator — heap-allocation явно, 5) `@heap` — реализовано. Регрессия: 1853/1853 ✓ (1586+267, +8 heap тестов).
> 2026-06-10: **docs: remove @embedded.noHeap** — Декоратор был документирован в spec, но **никогда не реализован** в компиляторе. Обсуждение выявило: (1) декораторы в нашем языке = императив, не декларация, (2) `noHeap` как маркер "не использует heap" был бы лишним (compile-time не нужен, runtime не имеет смысла без dynamic allocator switch), (3) `@heapFn` (compile-time guard "требует heap") лишний — `@heap` class decorator + `allocator` уже дают эту защиту, (4) `@onHeap` (runtime check) лишний — нет runtime API для переключения allocator. Решение: существующая защита через `allocator: "static"` + `@heap` class decorator достаточна. **Удалено:** `spec/11-concurrency.md` (секция целиком + строка в таблице + строка в ASCII diagram), `SPEC.md` (строка в таблице), `spec/13-platform-capabilities.md` (строка в migration). **Не реализовано:** новых декораторов (`@heapFn`, `@onHeap`) не нужно. **Регрессия:** не затронута (только docs).
